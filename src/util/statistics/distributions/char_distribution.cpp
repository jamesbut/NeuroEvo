...
CharDistribution::CharDistribution(const std::set<char>& char_set,
    ...
    //the character set
    if (char_distr)
        _distr = std::discrete_distribution<unsigned>(char_distr->begin(),
                                                      char_distr->end());
    else {
...
CharDistribution* CharDistribution::clone_impl() const
{
    return new CharDistribution(*this);
}

JSON CharDistribution::to_json_impl() const
{
    JSON json;
    json.emplace("name", "CharDistribution");
    json.emplace("char_set", _char_set);
    json.emplace("probabilities", _distr.probabilities());
    return json;
}
